...
async def test_create_expert_endpoint(pg, provider_env):
    ...
    resp = await client.post(
        "/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "ep-1"}
    )
...
async def test_create_requires_idempotency_key(pg, provider_env):
    ...
    resp = await client.post("/api/experts", json=EXPERT_CONTENT)
...
async def test_full_flow_create_edit_submit(pg, provider_env):
    ...
    created = (
        await client.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "f1"})
    ).json()["data"]
    ...
    # 详情可见 revision 状态
    detail = await client.get(f"/api/experts/{entity_id}")
...
async def test_without_entitlement_403(pg, provider_env):
    ...
    resp = await client.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "e1"})
    assert resp.status_code == 403
...
async def test_get_cross_owner_404(pg, provider_env):
    ...
    created = (
        await client_a.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "o1"})
    ).json()["data"]
    client_b, _ = await _author_client(pg, "api-other@x.com")
    resp = await client_b.get(f"/api/experts/{created['entity']['id']}")
...
async def test_expert_unknown_field_400(pg, provider_env):
    ...
    resp = await client.post(
        "/api/experts", json=dict(EXPERT_CONTENT, hacker=1), headers={"Idempotency-Key": "v2"}
    )
    assert resp.status_code == 400, resp.text
    assert resp.json()["error"]["code"] == "VALIDATION_ERROR"
...
async def test_skill_full_flow(pg, provider_env):
    ...
    created = (
        await client.post("/api/skills", json=SKILL_CONTENT, headers={"Idempotency-Key": "s1"})
    ).json()["data"]
    assert created["entity"]["status"] == "draft"
    entity_id = created["entity"]["id"]
    revision_id = created["revision"]["revision_id"]
    submitted = await client.post(
        f"/api/skills/{entity_id}/revisions/{revision_id}/submit",
        json={"tools": []},
...
async def test_list_status_filter_endpoint(pg, provider_env):
    ...
    created = (
        await client.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "l1"})
    ).json()["data"]
    ...
    resp = await client.get("/api/experts", params={"status": "draft"})
    assert resp.status_code == 200, resp.text
...
async def test_get_invalid_uuid_400(pg, provider_env):
    ...
    resp = await client.get("/api/experts/not-a-uuid")
    assert resp.status_code == 400
    assert resp.json()["error"]["code"] == "VALIDATION_ERROR"
...
async def test_submit_unknown_tool_400_endpoint(pg, provider_env):
    ...
    created = (
        await client.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "t1"})
    ).json()["data"]
    resp = await client.post(
...
async def test_submit_idempotent_replay(pg, provider_env):
    ...
    created = (
        await client.post("/api/experts", json=EXPERT_CONTENT, headers={"Idempotency-Key": "r1"})
    ).json()["data"]
    submit_path = (
        f"/api/experts/{created['entity']['id']}"
        f"/revisions/{created['revision']['revision_id']}/submit"
...
async def test_skill_detail_shape(pg, provider_env):
    ...
    created = (
        await client.post("/api/skills", json=SKILL_CONTENT, headers={"Idempotency-Key": "d1"})
    ).json()["data"]
    detail = await client.get(f"/api/skills/{created['entity']['id']}")
